...
def dia_GaussianW2(m0,m1,Sigma0,Sigma1):
    # compute the quadratic Wasserstein distance between two Gaussians with means m0 and m1 and scaled Sigma0 and Sigma1 such that a * Sigma0 = b * Sigma1
    return np.linalg.norm(m0-m1)**2+np.sum([Sigma0[i,i]+Sigma1[i,i]-2*np.sqrt(Sigma0[i,i]*Sigma1[i,i]) for i in range(len(Sigma0))])
    #return d
...
def GW2(pi0,pi1,mu0,mu1,S0,S1,regul=0,unbalanced=False,dia=False):
    ...
    for k in range(K0):
        for l in range(K1):
            M[k,l]  = GaussianW2(mu0[k,:],mu1[l,:],S0[k,:,:],S1[l,:,:]) if not dia else dia_GaussianW2(mu0[k,:],mu1[l,:],S0[k,:,:],S1[l,:,:])
    ...
    if regul > 0:   
        if unbalanced:
            ...
            wstar = ot.unbalanced.mm_unbalanced(pi0,pi1,M,regul)
        else:
            wstar     = ot.sinkhorn(pi0,pi1,M,regul)
    else:
        wstar     = ot.emd(pi0,pi1,M)         # discrete transport plan
    distGW2   = np.sum(wstar*M) #distance
    return distGW2, wstar


def tied_scaled_GW2(pi0,pi1,mu0,mu1,S0,S1,regul=0,unbalanced=False):
    # return the GW2 discrete map and the GW2 distance between two GMM
    K0 = mu0.shape[0]
    K1 = mu1.shape[0]
    d  = mu0.shape[1]
