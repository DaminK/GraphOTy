...
def cell_importance(adata, cell_type_overwrite=None, trends="cell_type", width=40, height=35, xlim=5, p_val=1, plot_cell=True, point_size=20, color_back='white',
                   fontsize=20, alpha=1, cmap_proportions='viridis', cmap_heatmap='Blues', save_as_pdf=True,
                   figsize=(12, 12), col_cluster=True, row_cluster=False, yaxis_label='cell proportions'):
    """
    ...
    cell_types = annot['cell_type'].unique() if cell_type_overwrite is None else pd.Series(cell_type_overwrite).unique()
    ...
    RNA_target = np.transpose(pathies_cell_proportions.iloc[:,1:len(cell_types)+1])
    ...
    if plot_cell:
        ...
        with plt.rc_context():
                plot_best_matches_cell_types(RNA_target, RNA_data,pathies_cell_proportions, sorted_best,  yaxis_label,
...
def Precomputed_distance(adata,distances,cost_df,features_matrix,emb_matrix='X_PCA',data_type='scRNA',
clusters_col='cell_types',sample_col='sampleID',status='status'):
    """
